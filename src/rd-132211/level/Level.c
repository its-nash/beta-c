...
void level_load(Level* level) {
    ...
    gzread(file, level->blocks,
           level->width * level->height * level->depth);
    gzclose(file);

    level_calcLightDepths(level, 0, 0,
    ...
    for (int x = x0; x < x0 + x1; x++) {
        for (int z = y0; z < y0 + y1; z++) {
            ...
            if (oldDepth != y) {
                ...
                levelrenderer_lightColumnChanged(
                    &level->renderer, x, z, yl0, yl1);
            }
        }
    }
}
...
int level_isTile(Level* level, int x, int y, int z) {
    if (x >= 0 && y >= 0 && z >= 0 &&
        ...
        return level->blocks[
            (y * level->height + z) * level->width + x
        ] == 1;
    }
    return 0;
}
...
float level_getBrightness(Level* level, int x, int y, int z) {
    ...
    if (x >= 0 && y >= 0 && z >= 0 &&
        ...
        return y < level->lightDepths[x + z * level->width]
               ? dark : light;
    }
